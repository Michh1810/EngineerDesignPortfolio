...
export default function AboutPage() {
  ...
  return (
    <main ref={containerRef} className="min-h-screen bg-[#070707] relative selection:bg-white/10 selection:text-white pb-24">
      ...
      <div className="relative z-10 max-w-2xl mx-auto px-6 pt-32 md:pt-48 flex flex-col">
        <div ref={contentRef} className="flex flex-col gap-16 md:gap-24">
          ...
          <section className="flex flex-col gap-6">
            <h1 className="text-h2 md:text-h1 font-medium text-[#F0EDE8] tracking-tight">
              Michael Truong
            </h1>
            <p className="text-body text-[#A09890] leading-relaxed">
              ...
              {experience.map((item, i) => (
                <div key={i} className="flex flex-col md:flex-row md:justify-between gap-1 md:gap-8 group">
                  <div className="flex flex-col md:w-2/3">
                    <h3 className="text-body font-normal text-[#F0EDE8]">
                      <span className="opacity-100">{item.company}</span>
                      <span className="opacity-70">, {item.role}</span>
                    </h3>
                    <span className="text-body text-[#F0EDE8] opacity-70 md:hidden mt-1">{item.year}</span>
                  </div>
                  <div className="hidden md:block md:w-1/3 text-right">
                    <span className="text-body text-[#F0EDE8] opacity-70">{item.year}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <section className="flex flex-col gap-4">
            ...
            <div className="flex flex-row flex-wrap items-center gap-3 text-body">
              <a href="https://www.linkedin.com/in/michael-truongg/" target="_blank" rel="noopener noreferrer" className="text-[#F0EDE8] hover-text-fade w-fit">LinkedIn</a>
              <span className="text-[#605850]">|</span>
              <a href="https://github.com/Michh1810" target="_blank" rel="noopener noreferrer" className="text-[#F0EDE8] hover-text-fade w-fit">GitHub</a>
              <span className="text-[#605850]">|</span>
              <a href="https://x.com/MikeSocTrang" target="_blank" rel="noopener noreferrer" className="text-[#F0EDE8] hover-text-fade w-fit">Twitter</a>
              <span className="text-[#605850]">|</span>
              <a href="mailto:michaeltruonggg@gmail.com" className="text-[#F0EDE8] hover-text-fade w-fit">michaeltruonggg@gmail.com</a>
            </div>
          </section>
