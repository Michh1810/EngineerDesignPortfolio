...
export default function ProgressLoader({ activeIndex, totalCards, isVisible = true }: ProgressLoaderProps) {
  ...
  return (
    <div
      className={`absolute top-8 md:top-auto md:bottom-12 left-1/2 z-30 -translate-x-1/2 flex flex-col items-center gap-2 transition-opacity duration-700 ease-out ${isVisible ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
        }`}
    >
